...
Skip this if the environment resolves markets from the operator key — that is the default
and nothing else in this runbook depends on it.

```bash
DRY_RUN=1 ./scripts/deploy-uma-adapter.sh <target>   # preflight only
./scripts/deploy-uma-adapter.sh <target>             # deploy + record in the manifest
```

Two things to know before running it, both consequences of `conditionId =
keccak256(oracle, questionId, 2)` — the resolver is part of a market's identity:
...
**Full procedure — prerequisites, per-market lifecycle, local testing, troubleshooting:
[uma-adapter.md](uma-adapter.md).**
